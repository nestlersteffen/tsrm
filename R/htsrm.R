...
#' Fit a bivariate Triadic Social Relations Model where the second (!) variable is a 
#' standard round-robin variable
#'
...
htsrm <- function( formula = NULL, g_var = NULL, p_var = NULL,   
	data = NULL, control = make_args_list(), debug = FALSE )
{
	...
	#- finally...
	result <- c( result, model="htsrm", list( names_list=names_list, names_y=names_y ) ) 
	class( result ) <- "tsrm"
  	return( result )
}